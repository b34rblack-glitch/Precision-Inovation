...
export async function createWorkup(params: {
  ...
  // One transaction: a workup used to be committable without the charge steps
  // that give it meaning.
  mutate((tx) => {
    insertRow(tx, 'workups', row);
...
export async function updateWorkupStatus(
  ...
  mutate((tx) => updateRow(tx, 'workups', id, { status }));
}
...
export async function updateStepResult(
  ...
  mutate((tx) => updateRow(tx, 'workupSteps', stepId, data));
}
...
export async function archiveWorkup(id: string): Promise<void> {
  mutate((tx) => {
    const [existing] = tx
      ...
      .from(workups)
      .where(and(eq(workups.id, id), isNull(workups.archivedAt)))
      .all();
    if (!existing) return;
    updateRow(tx, 'workups', id, { archivedAt: now() });
  });
}
...
export async function promoteChargeToVersion(workupId: string, chargeGr: number): Promise<string> {
  ...
  // One transaction across the version insert and both pointer updates: a crash
  // between them used to leave a promoted version that no load pointed at, or a
  // workup marked complete with no result.
  return mutate((tx) => {
    const [workup] = tx.select().from(workups).where(eq(workups.id, workupId)).all();
    if (!workup) throw new Error(`Workup ${workupId} not found`);
    const [load] = tx.select().from(loads).where(eq(loads.id, workup.loadId)).all();
    if (!load) throw new Error(`Load ${workup.loadId} not found`);

    const baseId = workup.baseVersionId ?? load.currentVersionId;
    const base = baseId
      ? tx.select().from(loadVersions).where(eq(loadVersions.id, baseId)).all()[0]
      : undefined;
    const latest = tx
      .select()
      .from(loadVersions)
    ...
    const [step] = tx
      ...
      .where(and(eq(workupSteps.workupId, workupId), eq(workupSteps.chargeGr, chargeGr)))
      .all();
    const measuredMv = step
      ? (tx.select().from(shotStrings).where(eq(shotStrings.workupStepId, step.id)).all()[0]
          ?.avgFps ?? null)
      : null;
    ...
    updateRow(tx, 'workups', workupId, {
      resultLoadVersionId: versionId,
      status: 'complete',
    });
    updateRow(tx, 'loads', load.id, { currentVersionId: versionId });
